...
#ETCDCTL_API=3 etcdctl --cert="/etc/etcd/peer.crt" --key="/etc/etcd/peer.key" --cacert="/etc/etcd/ca.crt"  --endpoints https://`hostname`:2379 snapshot save /nas_etcd/etcd_backup/`hostname -s`/etcd-data-$(date +%Y%m%d)/db
#else
